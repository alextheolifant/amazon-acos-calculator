...
export default function Home() {
  ...
  const canCalculate = useMemo(() => {
    return Number.isFinite(adSalesNum) && Number.isFinite(adSpendNum) && adSalesNum > 0;
  }, [adSalesNum, adSpendNum]);

  const acos = useMemo(() => {
    if (!canCalculate) return NaN;
    return (adSpendNum / adSalesNum) * 100;
  }, [canCalculate, adSpendNum, adSalesNum]);
        ...
        <div className="mx-auto max-w-5xl px-4 pt-12 pb-16">
          <div className="text-center">
            ...
            <div className="mt-4 text-lg text-gray-600 leading-snug">
              ...
              <div className="mt-1">Enter your ad sales and ad spend, then click calculate.</div>
            </div>
          </div>
                ...
                {showResult ? (
                  <div className="mt-6 rounded-2xl border border-gray-100 bg-[#fbf6e7] p-6 text-center">
                    ...
                    <div className="mt-2 text-5xl font-bold text-gray-900">
                      {canCalculate ? formatPct(acos) : "—"}
                    </div>
                    <div className="mt-2 text-sm text-gray-700">
                      Formula: <span className="font-semibold">(Ad Spend ÷ Ad Sales) × 100</span>
                    </div>
